...
//	}
	...
	public void loginBuyer() {
		...
		try {
			if(usr.checkAdmin(eid)) {
				//check if password is right. if not try again
				if(usr.checkAdminPass(pwd)) {
					//do something
					AdminHome ah = new AdminHome(usr, prod);
					ah.setVisible(true);
				...
				AdminNotExist notExistDialog = new AdminNotExist(usr);
